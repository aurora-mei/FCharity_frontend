...
const InviteRequestScreen = () => {
  ...
  const {
    ...
    error: projectError,
  } = useSelector((state) => state.inviteRequestProject || initialProjectState);
  ...
  console.log("myOrgInvitations", myOrgInvitations);
  ...
  useEffect(() => {
    if (projectSuccess && currentUser?.id) {
      message.success(
        projectSuccess.message || "Project action completed successfully!"
      );
  ...
  useEffect(() => {
    if (orgSuccess && currentUser?.id) {
      message.success(
        orgSuccess.message || "Organization action completed successfully!"
      );
      setActionLoadingId(null);
  ...
  const handleApprove = (id, type) => {
    ...
    if (type === "project") {
      ...
      dispatch(acceptInvitationRequest(id));
  ...
  const handleReject = (id, type) => {
    ...
    if (type === "project") {
      ...
      dispatch(rejectInvitationRequest(id));
      // dispatch(rejectOrgInvitationThunk(id));
    }
  };

  const dataSource = useMemo(() => {
    const mappedProjectInvites =
    ...
    return [...mappedProjectInvites, ...mappedOrgInvites].sort(
