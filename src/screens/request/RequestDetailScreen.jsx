...
const RequestDetailScreen = () => {
  ...
  const { ownedOrganization, currentOrganizationMembers } = useSelector(
  ...
  const imageUrls =
    requestData.attachments?.filter((url) =>
      url.match(/\.(jpeg|jpg|png|gif)$/i)
    ) || [];
  const videoUrls =
    requestData.attachments?.filter((url) => url.match(/\.(mp4|webm|ogg)$/i)) ||
    [];
      ...
      <Flex gap={10} vertical className="request-detail-page">
        ...
        <Flex vertical gap={10} className="request-detail">
          ...
          {expanded ? (
            ...
            <Paragraph>{`${helpRequest.content.substring(
              0,
              800
            )}...`}</Paragraph>
          )}
                  ...
                  if (ownedOrganization) {
                    if (ownedOrganization.organizationStatus === "APPROVED") {
                      if (
                        ...
                        message.error(
                          "Your organization doesn't have any available member to be leader"
                        );
                      } else {
                        navigate(
                    ...
                    message.error(
                      "You must be a ceo of an organization to register for request"
                    );
                  }
                }}
              >
