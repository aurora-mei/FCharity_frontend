import {
  ...
  const AddProjectMemberScreen = () => {
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        if (isFirstMount && currentOrganizationMembers?.length > 0 && newProject?.project) {
          setAvailableMembers(
            currentOrganizationMembers.filter(
              (x) =>
                x.memberRole === "MEMBER" &&
                newProject.project.leader.id !== x.user.id
            )
          );
          setIsFirstMount(false);
        }
      }, [currentOrganizationMembers, newProject,myProjectMembers]);
        ...
        >
          <Col span={24}>
          {
              ...
              <Flex vertical gap={20} justify="flex-start">
                ...
                >
                  Navigate to {ownedOrganization && ownedOrganization.organizationName.toUpperCase()} 
                </StyledButton>
              </Flex>
           )
          }
