...
const OtpVerification = () => {
  ...
  const [email, setEmail] = useState(
    useSelector((state) => state.auth.newUser.email) ?? ""
  );
  ...
  return (
    <div className="otp-container">
      {!email && (
        <Form layout="vertical" className="otp-box" onFinish={sendOTPRequest}>
          <Form.Item
            label="Email Address"
            name="email"
            rules={[
      ...
      {email && (
        <Form layout="vertical" className="otp-box" onFinish={verifyRequest}>
          ...
          <Text className="otp-description">
            Your safety is our top priority. Please enter the code we sent to{" "}
            {email}
          </Text>
          <Form.Item
          ...
          {error && (
            <Text type="danger" className="otp-error">
              {error}
            </Text>
          )}
